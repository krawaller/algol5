...
export default function parseUniversal(gameDef: Definition, player: 1 | 2, action: string, parser, expression, from){
  const parse = makeParser(gameDef, player, action, from);
  const me = (expr) => parseUniversal(gameDef, player, action, parser, expr, from);
