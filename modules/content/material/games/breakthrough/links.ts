export const links = {
  "World of Abstract Games entry":
    "http://www.di.fc.ul.pt/~jpn/gv/breakthrough.htm",
  "Board Game Geek entry":
    "https://boardgamegeek.com/boardgame/3825/breakthrough",
  "Zillions of Games entry":
    "http://www.zillionsofgames.com/cgi-bin/zilligames/submissions.cgi?do=show;id=35",
  "IG Game Center entry":
    "http://www.iggamecenter.com/info/en/breakthrough.html",
  "Super Duper Games entry":
    "http://superdupergames.org/gameinfo.html?game=break",
  "Wikipedia entry":
    "https://en.wikipedia.org/wiki/Breakthrough_%28board_game%29",
  "Little Golem entry":
    "http://www.littlegolem.net/jsp/games/gamedetail.jsp?gtid=brkthr&page=rules",
  "Abstract Meanderings entry":
    "https://abstractmeanderings.wordpress.com/2014/10/05/breakthrough-game-2/",
};
