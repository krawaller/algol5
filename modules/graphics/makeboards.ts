...
function draw(game: FullDefAnon, to) {
  ...
  corner.src = pics.corner_nw;
  ctx.drawImage(corner, 0, 0, EDGE, EDGE);
  corner = new Image();
  corner.src = pics.corner_ne;
  ctx.drawImage(corner, board.width * TILE + EDGE, 0, EDGE, EDGE);
  corner = new Image();
  corner.src = pics.corner_se;
  ctx.drawImage(
    corner,
    board.width * TILE + EDGE,
    board.height * TILE + EDGE,
    EDGE,
    EDGE
  );
  corner = new Image();
  corner.src = pics.corner_sw;
  ctx.drawImage(corner, 0, board.height * TILE + EDGE, EDGE, EDGE);

  for (let col = 1; col <= board.width; col++) {
    let colHeader = new Image();
    ...
    ctx.fillText(
      colnumber2name[col],
      (col - 1) * TILE + EDGE + TILE / 2,
      EDGE / 2
    );
    ctx.fillText(
      colnumber2name[col],
      (col - 1) * TILE + EDGE + TILE / 2,
      board.height * TILE + (EDGE * 3) / 2
    );
  }
  for (let row = 1; row <= board.height; row++) {
    let picY = EDGE + (board.height - row) * TILE;
    let rowHeader = new Image();
    ...
    ctx.fillStyle = "white";
    ctx.fillText(row.toString(), EDGE / 2, picY + TILE / 2);
    ctx.fillText(
      row.toString(),
      board.width * TILE + (EDGE * 3) / 2,
      picY + TILE / 2
    );
    for (let col = 1; col <= board.width; col++) {
