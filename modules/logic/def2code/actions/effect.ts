...
function executeEffect(
  ...
  switch (type) {
    case "swap": {
      ...
      return (
        executeEffect(gameDef, player, action, [
          "setat",
          pos1,
          "pos",
          ["pos", pos2]
    ...
    case "moveid": {
      ...
      */
      const [id, pos] = args;
      return executeEffect(gameDef, player, action, [
        "setid",
        id,
        "pos",
        ["pos", pos]
    ...
    case "killat": {
      ...
      */
      const [pos] = args;
      return `delete UNITDATA[ (UNITLAYERS.units[${expr.position(
        pos
      )}] || {}).id ]; `;
    }
    case "killin": {
      ...
      return `
        ...
        UNITDATA[newunitid] = {
          ...
          ${
            obj
              ? "," +
                map(obj, (val, key) => key + ":" + expr.value(val)).join(",")
              : ""
          }
        }; 
      `;
