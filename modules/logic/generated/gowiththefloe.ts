import fullDef from "../../games/dist/games/gowiththefloe";
import {
  relativedirs,
  reduce,
  pos2coords,
...

{
  ...
  game.selectunit1instruction = function(turn, step) {
    return collapseLine({
      type: "line",
      content: [{ type: "text", text: "Select where to move" }]
    });
  };
  ...
  game.selectmovetarget1instruction = function(turn, step) {
    return collapseLine({
      ...
      content: [
        ...
        { type: "text", text: "to go here" }
      ]
    });
  };

  game.selecteattarget1 = function(turn, step, markpos) {
    let MARKS = { selecteattarget: markpos, selectunit: step.MARKS.selectunit };
  ...
  game.selecteattarget1instruction = function(turn, step) {
    return collapseLine({
      ...
      content: [
        { type: "text", text: "Press" },
        { type: "cmndref", cmnd: "eat" },
        { type: "text", text: "to, well, eat" }
      ]
    });
  };
  ...
  game.move1 = function(turn, step) {
    ...
    for (let STARTPOS in walkstarts) {
      ...
      for (let walkerdirnbr = 0; walkerdirnbr < 8; walkerdirnbr++) {
        ...
        let walkpositionstocount = (function() {
          let ret = {},
            s0 = TERRAIN.nowater,
            s1 = UNITLAYERS.holes;
          for (let key in s0) {
    ...
    if (
      ...
      turn.endMarks[newstepid].safeseal = (function() {
        let ret = {},
          s0 = UNITLAYERS.seals,
