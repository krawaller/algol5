...

{
  ...
  game.brain_Steve_1 = function(step) {
    ...
    }
    {
      ...
      for (let POS in filtersourcelayer) {
        let filtertargetlayer =
          ARTIFACTS[
            !!UNITLAYERS.oppwalls[POS] ? "oppheavythreats" : "opplightthreats"
          ];
    ...
    return (
      2 * Object.keys(UNITLAYERS.mytowers).length +
      ...
      20 * Object.keys(ARTIFACTS.opplightthreats).length -
      500 * Object.keys(ARTIFACTS.oppheavythreats).length
    );
  };
  ...
  game.brain_Clive_1 = function(step) {
    ...
    return (
      Object.keys(ARTIFACTS.mymoves).length +
      3 *
        Object.keys(UNITLAYERS.mytowers).reduce(function(mem, pos) {
          return mem + (mybasic[pos] || 0);
        }, 0) +
