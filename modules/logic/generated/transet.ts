...
};
{
  ...
  game.selectunit1 = function(turn, step, markpos) {
    ...
    let neighbourdirs = (!!(UNITLAYERS.pinets[MARKS["selectunit"]]) ? [1] : (!!(UNITLAYERS.piokers[MARKS["selectunit"]]) ? [8, 2] : [8, 1, 2]));
  ...
  game.selectswap1target1 = function(turn, step, markpos) {
    ...
    let POS = connections[STARTPOS][relativedirs[(ARTIFACTS.swap1steps[MARKS["selectswap1target"]] || {})["dir"] - 2 + 5]];
    if (POS && !
      (function() {
        let k, ret = {},
          s0 = UNITLAYERS.units,
...
};
{
  ...
  game.selectunit2 = function(turn, step, markpos) {
    ...
    let STARTPOS = MARKS["selectunit"];
    let neighbourdirs = (!!(UNITLAYERS.pinets[MARKS["selectunit"]]) ? [5] : (!!(UNITLAYERS.piokers[MARKS["selectunit"]]) ? [4, 6] : [4, 5, 6]));
    let nbrofneighbourdirs = neighbourdirs.length;
    let startconnections = connections[STARTPOS];
    for (let dirnbr = 0; dirnbr < nbrofneighbourdirs; dirnbr++) {
      let POS = startconnections[neighbourdirs[dirnbr]];
